...
### 10. Answers view — expandable raw response text in report viewer (2026-08-02)
Added directly after the ChatGPT-empty-answer investigation surfaced that there was no way to see an engine's actual response anywhere in the app (Known Bug #8's "related gap") — only a direct Supabase query could show it. `GET /api/reports/[runId]` now also selects `raw_answer` and `created_at` from `answers` and returns them as `text`/`createdAt` on each answer object (`app/api/reports/[runId]/route.ts`). In `ReportViewer`'s "Prompt results" table (`app/app/page.tsx`), each row is now clickable — expands to a row showing the full response text and when it was answered (`toLocaleString` with date+time, the first place in the app that shows time-of-day, not just a date). Kept the expand markup `no-print` so the existing PDF export layout is unaffected. Reused the existing `Fragment`-per-row pattern (needed for a keyed sibling pair inside `.map`), the existing `ChevronDown` icon (already imported, just rotated via inline transform), and the existing `.status.yes/.no` badge classes — no new components or CSS added.

### 11. Reports, SEO Audit & Traffic upgrade (2026-08-04)
An 8-part request: timestamps everywhere, Google traffic with date filters, a complete multi-tab SEO audit, reports that embed the full audit + traffic + historical comparison, and a score-trend chart — while preserving everything that already worked. Planned in full (`.claude/plans` — approved) before any code, staged bottom-up: migration → shared utilities → API routes → UI, each stage typechecked/tested/built independently.

**Four scope-defining decisions, made before implementation (all picked the recommended option):**
1. **Performance/Accessibility** — new Google PageSpeed Insights integration (`lib/seo/pagespeed.ts`), gated on `PAGESPEED_API_KEY` same as every other optional provider in this codebase (`if(process.env.X_API_KEY)`). Unset key → those two tabs say so honestly instead of showing fabricated scores.
2. **Traffic metrics** — Search Console only (clicks/impressions/CTR/position). No GA4 — would've meant a second OAuth integration for sessions/users data nothing else in the app needs yet.
3. **Traffic caching** — session-level only (client-side `Map` keyed by range, cleared on brand switch/disconnect), no new DB table. Google's Search Analytics API already serves arbitrary date ranges live; a snapshot table would just be a sync problem to maintain.
4. **Audit issue status** — auto-derived (pass/warning/fail from the latest audit + fixed/broken/unchanged vs. the previous one), no manual "mark resolved" workflow. Comparing check-by-id status between the two most recent `seo_audits` rows is the entire model.

**Schema (`supabase/schema.sql`) — needs a manual migration, see `PLAN.md` Known debt #7:** new `seo_audits` table (`brand_id`, `domain`, `checks` jsonb array, `overall_score`, `created_at`), one index (`brand_id, created_at desc`), RLS mirroring the existing `fixes`-joins-through-`brands` pattern. Deliberately **no** `score` column on `scan_runs` — `score()` in `lib/ai/scoring.ts` is a pure, sub-millisecond function already recomputed live everywhere; storing it would risk silently going stale if the scoring algorithm ever changes, for no read-speed benefit `/api/scan-history` doesn't already have.

**New shared modules** (all reused by 2+ call sites, not duplicated):
- `lib/format/datetime.ts` — `formatTimestamp(iso, "date"|"datetime")`, the one place every timestamp in the app now goes through. Before this there were three different `toLocaleDateString` option sets for what was conceptually the same "scan date," and nothing showed time-of-day.
- `lib/data/reportComparison.ts` — `compareToPrevious(history, runId)`. Baseline/delta math used to be copy-pasted slightly differently at three call sites (Reports card list, `ReportViewer` header, Overview's `ScoreHero`); now there's one function and one behavior — including "no previous report" returning `null` fields instead of a fake zero delta.
- `lib/seo/checks.ts` — the ~200-line regex audit extracted unchanged from `app/api/seo-audit/route.ts` (which now just calls it), so the original live-only endpoint and the new persisting one run identical checks against identical HTML. Recategorized checks onto the 10-tab layout while extracting: `og_tags`/`twitter_card` moved `social`→`meta` (no "Social" tab in the spec), `viewport` moved `technical`→`mobile`, `internal_links` moved `content`→`links`.
- `lib/seo/pagespeed.ts` — PageSpeed Insights client + `pageSpeedToChecks()`, which folds Lighthouse scores into the *same* `CheckResult` shape as the regex checks (performance/LCP/CLS/best-practices/accessibility as synthetic checks with their own pass/warning/fail). Means the Recommendations tab and Overview counts don't need a special case for "checks that came from Google instead of a regex."
- `lib/seo/latestAudit.ts` — `getLatestAuditWithTrend()`, shared by `/api/seo-audit/latest` and `/api/reports/[runId]` so both compute fixed/broken/unchanged the same way.
- `lib/gsc/metrics.ts` — `fetchGscTraffic()` + range resolution, extracted from `/api/gsc/metrics` so `/api/reports/[runId]` can embed the same live Search Console data without reimplementing the OAuth-refresh-then-query dance.
- `app/components/Tabs.tsx` — the first tab bar in this app with real ARIA (`role="tablist"/"tab"/"tabpanel"`, arrow-key navigation). Every prior tab bar (Overview, Fixes, Settings) was a plain `<button>` array; fine at 2-3 tabs, not at the 10 the SEO Audit needed. Visually still just the existing `.overview-tabs` class.
- `app/components/ScoreTrendChart.tsx` — hand-rolled SVG line chart (gridlines, axis labels, hover tooltip), same technique as the existing `Sparkline`, no charting library added. Shared by the report's score-history section.

**API routes:** `POST /api/seo-audit/run` (runs regex + PageSpeed in parallel via `Promise.all`, persists, 6-hour in-memory rate limit per user+brand — same pattern as `/api/scan`'s rate limiter, not the Postgres-backed one which is reserved for the *unauthenticated* endpoints); `GET /api/seo-audit/latest` and `/history` (paginated, `cursor`-based); `/api/scan-history` gained a bounded `limit` param (expensive part — the per-answer payload and `score()` calls — is now capped even though finding *which* runs are the most recent N still can't be index-bounded without a DB function, since `scan_runs` has no `brand_id` of its own); `/api/gsc/metrics` gained `range=7d|30d|90d|custom&start&end`, `rowLimit` now scales to the actual span instead of a hardcoded 28; `/api/reports/[runId]` now embeds `seoAudit` and `traffic` (fetched in parallel with the existing fix query) and returns `created_at` on each fix (was selected nowhere before).

**UI (`app/app/page.tsx`):** SEO Audit tab replaced entirely — 10 tabs (Overview/Technical SEO/On-page SEO/Content/Performance/Mobile/Accessibility/Links/Structured Data/Recommendations) via the new `Tabs` component, each check row shows a Fixed/New pill when its status changed since the last audit. GSC traffic tab gained range buttons + native `<input type="date">` for custom range + a `Map`-based session cache keyed by range so switching tabs doesn't refetch. `ReportViewer` gained: a score-history section (`ScoreTrendChart` over the existing `history` prop — no new fetch), a richer delta line (percent change + previous score + previous date, not just absolute points), an embedded SEO audit section (`<details open>` per category — collapsible on-screen, forced open in print via new `globals.css` rules so PDF export always has the complete audit regardless of what the viewer collapsed), and an embedded traffic section (30-day snapshot, live).
