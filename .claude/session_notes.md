...
### Short-term (open)
- [ ] **RUN THE RATE-LIMIT MIGRATION.** `2ea03b0` ships a Postgres-backed limiter for `/api/check` and `/api/audit`, but the table and function only exist in `supabase/schema.sql` — they are not in the live database yet. Until you run them in the Supabase SQL editor, `consume_rate_limit` doesn't resolve, the helper fails closed, and both endpoints return `503`. Confirmed live: `POST /api/check` → `503 {"error":"The visibility checker is temporarily unavailable…"}`. No user-visible impact today, since neither endpoint has an in-app caller. Copy the `public.rate_limits` table, the `consume_rate_limit` function, and the `revoke` line from the bottom of `supabase/schema.sql`.
