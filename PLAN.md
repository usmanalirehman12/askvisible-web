...
### ~~7. Audit trail + AI Fixes progress report~~ — DONE 2026-08-06, migration run
...
gained an "AI Fixes progress" sub-tab showing each fix's full status history. Migration
(`audit_log` table + `fixes.scan_run_id` column) run in Supabase 2026-08-06 — live in
production. Full detail in `.claude/session_notes.md` #15.
...
| 8 | `audit_log` table + `fixes.scan_run_id` migration must be run manually | Resolved 2026-08-06 (you) — table + column + RLS created, live in production |
